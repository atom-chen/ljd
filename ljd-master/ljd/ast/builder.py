...
def _build_function_definition(prototype):
    ...
    state.instructions = prototype.instructions
...
def _build_statement(state, addr, instruction):
    if gconfig.gVerbose:
        print("_build_statement addr={addr}, instruction={instruction}".format(
...
def _build_call_arguments(state, addr, instruction):
    ...
    is_variadic = (instruction.opcode ==
                   ins.CALLM.opcode or instruction.opcode == ins.CALLMT.opcode)
...
def _lookup_variable_name(state, addr, slot):
    while True:
        result = _lookup_variable_name_step(state, addr, slot)
...
def _lookup_variable_name_step(state, addr, slot):
    ...
    constants = state.constants.complex_constants
    ...
    while addr > 1:
        ...
        if instruction.opcode == ins.TGETS.opcode:
            ...
            table = _lookup_variable_name_step(state, addr, table_slot)
        ...
        if instruction.opcode == ins.UGET.opcode:
            uv = instruction.CD
            name = state.debuginfo.lookup_upvalue_name(uv)
            if name is None:
                return "uv" + str(uv)
            else:
        ...
        # KSTR {A} = {D}
        if instruction.opcode == ins.KSTR.opcode and slot == instruction.A:
            return constants[instruction.CD]
